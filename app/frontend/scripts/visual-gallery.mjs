...
try {
    ...
    await capture(page, '08-data-sources', 'Data & Analytics / Sources');
    ...
    await capture(page, '09-data-operations', 'Data & Analytics / Operations');
    ...
    await capture(page, '10-data-metadata', 'Data & Analytics / Metadata');
    ...
    await capture(page, '11-artifacts-library', 'Artifacts / Library');
    await clickRouteTab(page, 'Artifacts main surface', 'Metadata');
    await capture(page, '12-artifacts-metadata', 'Artifacts / Metadata');
    ...
    await capture(page, '15-settings-connectors', 'Settings / Connectors');

    await page.keyboard.press('ControlOrMeta+Shift+P');
    await page.getByPlaceholder('Run command...').fill('git');
    await capture(page, '16-command-palette', 'Command palette');
    await page.keyboard.press('Escape');

    await page.keyboard.press('ControlOrMeta+P');
    await page.getByPlaceholder('Open file, folder, dataset, artifact...').fill('brief');
    await capture(page, '17-quick-open', 'Quick open');
    await page.keyboard.press('Escape');

    await page.setViewportSize({width: 390, height: 844});
    await page.locator('[data-studio-route="code"]').click();
    await page.locator('.editor-tabs').waitFor({state: 'visible'});
    await capture(page, '18-mobile-workbench', 'Mobile / Workbench');
    await page.locator('[data-studio-route="data"]').click();
    await capture(page, '19-mobile-data', 'Mobile / Data & Analytics');
    await page.locator('[data-studio-route="settings"]').click();
    await capture(page, '20-mobile-settings', 'Mobile / Settings');

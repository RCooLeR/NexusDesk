...
- Task-aware model defaults are now stored and editable in Settings for coding, backend, database, analytics, research, vision/screenshot, balanced reasoning, and fast-coding routes while preserving a global fallback. Ask mode exposes a route selector, Git AI diff summary/commit drafting resolves the main coding route, Agent mode resolves the selected route with audit provenance, and saved chat-answer artifacts can preserve route metadata; Data/document/vision route-aware workflow resolution remains planned.

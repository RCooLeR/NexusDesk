...
- inspect artifact provenance health in Diagnostics so generated-output release smoke catches missing metadata sidecars and weak lineage signals before sharing artifacts;
...
- inspect the native tool catalog, let agent runs read local dataset profile/query/SELECT-only SQL context and generate approval-gated dataset chart artifacts, inspect/query workspace SQLite files through approval-gated read-only connections, extract workspace document text, inspect operations files read-only with redaction, generate approval-gated operations runbook artifacts from redacted evidence, inspect/list approval posture, redact sensitive text before cross-tool use, record explicit future-approval requests for multi-step plans, inspect optional external coding-agent CLI readiness for Codex, Claude Code, and OpenCode, create non-executing run plans with approval/audit/cancel/output-capture requirements, inspect/cancel durable jobs with redacted log tails and approval-gated cancellation, stage/unstage Git files and hunks, commit already-staged changes, create branches, resolve standard conflict markers, and discard unstaged tracked changes or explicitly delete untracked files through approval-gated rollback-backed Git/workspace tools, and run first-party approved terminal commands through explicit argv, rooted cwd, timeout/output caps, shell/path blocking, and high-risk tool audit;
...
- Agent tool-registry health checks that prevent planned tools from becoming executable without catalog status, risk controls, approval/audit expectations, tests, and Diagnostics visibility.
...
- macOS Keychain, web fetch, and connector TLS defaults are not hardened to the latest review standard.

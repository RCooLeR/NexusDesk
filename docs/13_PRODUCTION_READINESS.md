...
- IDE-grade editor baseline: first native lightweight syntax strategy, read-only highlighted syntax preview, Problems syntax diagnostics for JSON/Go/YAML/TOML, bounded workspace go-to-definition fallback, future active-editor inline styling and LSP/cross-file language-action decisions, and continued outline/go-to-symbol/local-definition/document-map/breadcrumb/split/find/format polish.

...
- Task-aware model defaults are now stored and editable in Settings for coding, backend, database, analytics, research, vision/screenshot, balanced reasoning, and fast-coding routes while preserving a global fallback. Git AI diff summary/commit drafting now resolves the main coding route through the assistant service and saved chat-answer artifacts can preserve route metadata; broader route-aware workflow resolution remains planned.

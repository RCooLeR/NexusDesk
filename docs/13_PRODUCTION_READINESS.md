...
- IDE-grade editor baseline: first native lightweight syntax strategy, read-only highlighted syntax preview, Problems syntax diagnostics for JSON/Go/YAML/TOML, bounded workspace go-to-definition fallback and references search, future active-editor inline styling and LSP/cross-file language-action decisions, and continued outline/go-to-symbol/local-definition/document-map/breadcrumb/split/find/format polish.
...
- Assistant quality parity: native Fyne now has weak-evidence warnings, retry/compare, Wails-compatible memory/profile storage, stale-source chat history warnings, Wails-compatible context-to-source fallback parsing, source/model footer diagnostics, line-aware citation refs, bounded citation snippets in saved answer artifacts, deterministic evidence-quality labels, curated model context sizing, loaded-model runtime context tuning, and save-latest-answer artifacts.

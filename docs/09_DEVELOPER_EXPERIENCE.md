...
`app/internal/workspace/preview.go` keeps text previews rooted and size-limited, decodes UTF-8, UTF-16, and Windows-1251 text variants, and renders common image/PDF files as capped data URLs for inline display. Chat context still accepts only text previews, so binary, image, and PDF content is not sent to the model as source text.

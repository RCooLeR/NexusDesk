...
Chat history stores the source paths attached to each user/assistant pair so saved answer artifacts can use the answer's original context instead of whatever happens to be pinned later. Persisted assistant answers now append a compact source list when context paths exist, and saved Markdown answer artifacts include the same source citations in their header and footer. The chat header can retry the latest answered prompt or ask the current model/settings to compare against the latest answer using the same source paths. Assistant messages without explicit source context show a weak-evidence warning, and the composer shows a missing-context warning when nothing selectable or pinned can ground the next answer.
...
`app/app_git.go` owns the Wails-facing Git API types and bridge methods, while `app/git_service.go` owns the first Git service facade. `GetGitStatus`, `GetGitFileDiff`, `PreviewGitFileAction`, `PreviewGitHunkAction`, and `ApplyGitHunkAction` keep their Wails contracts stable and dispatch to `GitService`, which runs bounded `git` commands against the active workspace root, detects the repository root, branch, short HEAD, ahead/behind text, porcelain changed-file rows, staged/unstaged groups, a capped staged diff, and a capped working-tree diff. `GetGitFileDiff` loads a capped read-only staged and unstaged diff for one selected changed file. `PreviewGitFileAction` plans stage/unstage commands and returns the command, current status, approval requirement, and message without mutating the repository. Hunk actions are narrower: the backend rebuilds the selected hunk patch from the current Git diff by path, diff kind, and hunk index; unstaged hunks can be discarded from the working tree and staged hunks can be reverted from the index only after the frontend approval modal confirms the high-risk action. No Git API performs full-file reset, checkout, or broad discard operations. The frontend does not run Git automatically on workspace open; the user must press Refresh git in the Git drawer or Workbench repository surface. Workbench consumes these through `useGitController`, which owns Git status, selected changed-file state, selected diff loading, stage/unstage preview state, hunk action preview/apply state, null-response normalization, and explicit refresh/actions. The bottom Git drawer tab owns selected changed-file review, directory-structured changed-file lists, preview-only stage/unstage controls, read-only staged/unstaged diffs, unified/split/diff-only modes, icon hunk navigation, hunk selection state, approval-backed hunk discard/revert controls, AI diff summaries, and AI commit-message drafts.

`app/app_tasks.go` owns the first read-only workspace task discovery pass. `ListWorkspaceTasks` scans bounded workspace paths, skips noisy folders such as `.git`, `.nexusdesk`, build output, dist output, and `node_modules`, parses `package.json` scripts, and detects Go module test commands. It only returns command labels, working directories, and source files; it does not run npm, Go, shells, or any external process. Task execution belongs to a later approved job/task runner.
...
- `app/frontend/src/features/shell/useGitController.ts` owns Git status refresh, selected changed-file state, selected-file diff loading, preview-only stage/unstage action planning, approval-backed hunk action calls, null-response normalization, and the manual-only Git refresh boundary.
...
- `app/frontend/src/features/shell/CodeStudioPanel.tsx` owns reusable Workbench session metrics, open tabs, workspace status, git branch/dirty summary, staged/unstaged changed-file groups, selected changed-file state, the Workbench search utility panel, read-only detected task listings, and placeholder queues for problems and code review.
- `app/frontend/src/features/shell/GitDiffPanel.tsx` owns the bottom-drawer Git tab for working-tree review, including selected changed-file state, directory-structured changed-file lists, preview-only stage/unstage controls, per-file read-only staged/unstaged diffs, unified/split rendering, hunk navigation, hunk selection, approval-backed hunk discard/revert controls, assistant summary/draft actions, and refresh controls.
...
- `app/frontend/src/features/shell/AgentChatCard.tsx` owns the expanded chat presentation, full conversation scroll area, OpenAI-style composer with absolute-positioned model and Ask/Agent controls, context pack list, retry/compare/save-answer action surface, weak-evidence and missing-context warnings, and delegates provider calls/history/artifact actions back to the shell.
...
- `app/frontend/src/features/shell/DataOperationsPanel.tsx` owns the Data & Analytics route surface for dataset profiling, query/chart/SQL workflows, read-only SQLite connector queries, Operations inspector, metadata browser/search, and workspace freshness controls.
...
- DuckDB-compatible analytics: Data & Analytics can run constrained read-only SQL over CSV previews.

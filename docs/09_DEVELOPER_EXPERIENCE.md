...
Native assistant orchestration now starts in `nexus-app/internal/services/assistant`. The Fyne assistant panel dispatches Ask-mode prompts through that service instead of calling provider transport directly, streams deltas back to the response view with `fyne.Do`, keeps the user's prompt entry untouched, and includes the currently selected file, directory, or workspace context path as a bounded model context pack. The same panel can switch to Agent mode, which runs `nexus-app/internal/services/agent` with the native deterministic tool dispatcher, shows only the last one or two model/tool activity messages while the run is active, writes fuller events into the Activity tab, and replaces the temporary tail with the final answer. Full multi-pin UI, persisted chat history, and tool-run audit persistence remain later native ports.

...
`app/app_tasks.go` owns the first read-only workspace task discovery pass. `ListWorkspaceTasks` scans bounded workspace paths, skips noisy folders such as `.git`, `.nexusdesk`, build output, dist output, and `node_modules`, parses `package.json` scripts, detects Go module test commands, and lists Docker Compose config-check tasks for compose files. It only returns command labels, working directories, and source files; it does not run npm, Go, Docker, shells, or any external process. Task execution belongs to a later approved job/task runner.
...
- `app/frontend/src/features/shell/CodeStudioPanel.tsx` owns reusable Workbench session metrics, open tabs, workspace status, git branch/dirty summary, staged/unstaged changed-file groups, selected changed-file state, the Workbench path/text/regex search utility panel with non-mutating replace previews, read-only detected task listings, and placeholder queues for problems and code review.
- `app/frontend/src/features/shell/GitDiffPanel.tsx` owns the bottom-drawer Git tab for working-tree review, including selected changed-file state, directory-structured changed-file lists, file stage/unstage controls, per-file read-only staged/unstaged diffs, unified/split rendering, hunk navigation, hunk selection, approval-backed hunk stage/unstage/discard/revert controls, assistant summary/draft actions, and refresh controls.

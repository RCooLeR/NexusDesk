...
The native Fyne migration ports these workspace behaviors capability-by-capability into `nexus-app/internal/services/workspace`. The native safe-write slice now keeps write path validation, text diff preview, append/apply, UTF-8/UTF-8 BOM/UTF-16/Windows-1251 encoding, and rollback snapshots in service files (`write.go`, `write_path.go`, `write_diff.go`, `write_encoding.go`, and `rollback.go`). Native file-operation services (`file_ops.go`, `file_ops_path.go`) cover file create, folder create, delete, copy, move, and rename with rooted validation, metadata guards, operation previews, and rollback records. Folder create rollback can remove the created empty folder while still rejecting rollback over pre-existing directory targets. The native navigator exposes quick action buttons plus tree-row secondary-click context menus for create file/create folder/cut/copy/paste/rename/delete, relative-path copy, and assistant-context selection; file clipboard paste is only an intent until paste time, then the same copy/move service validates target paths, avoids overwrites for copied files, creates rollback snapshots, and refreshes the tree. Its tree controls can expand the selected branch, collapse all branches, and opt into safe ignored-folder visibility while keeping `.nexusdesk` metadata and symlinks hidden. The native shell now uses a labeled product rail plus resizable editor/assistant and top/bottom workbench split panes, so bottom tools can be expanded when they are the active task without consuming the entire app. Native editor tabs track pinned ordering and dirty draft state through `internal/services/editor`; closing a modified tab now prompts for explicit discard instead of silently dropping or trapping the tab. The native draft editor Save action calls this service, marks the saved tab clean, and must not write directly to workspace files from UI code.
...
Native dataset profiling and row querying start in `nexus-app/internal/services/datasets`. The current slice profiles selected workspace CSV, TSV, JSON, NDJSON/JSONL, XLSX, log, and Parquet files through rooted safe reads. CSV/TSV/XLSX profiles report sample row counts, field names, inferred lightweight types, empty/non-empty counts, sample values, XLSX sheet names, and truncation when the preview cap applies; JSON and NDJSON profiles report top-level shape and object fields; log profiles expose line, level, and message columns with detected level notes. Parquet profiling validates `PAR1` header/footer magic, reads only bounded footer bytes, and decodes the Thrift Compact footer metadata needed for schema column names/types, file row count, row groups, column chunk codecs, value counts, and compressed/uncompressed byte summaries. The explicit dependency decision is to keep this metadata path native and dependency-free for now; full Parquet value reads remain deferred until a reader/import dependency is chosen. `nexus-app/internal/services/spreadsheets` reads bounded XLSX workbook rows from OpenXML package parts without pulling spreadsheet parsing into UI code or running external converters. The same dataset service now runs bounded row queries over preview-safe CSV, TSV, JSON, NDJSON/JSONL, first-sheet XLSX, and log rows with global text search, `column=value`, `column contains value`, numeric comparisons, `order by column asc|desc`, and `limit N`; query output is capped to 50 displayed rows and reports loaded, matched, and shown counts. The first native SQL action is a constrained SELECT-only adapter over the selected dataset: it supports projection, one `WHERE` predicate, `ORDER BY`, `LIMIT`, a deterministic execution plan, read-only mutation blocking, and metadata persistence for SQL runs plus dataset dependencies. The native SQL notebook model persists per-dataset notebooks under `.nexusdesk/datasets/notebooks.json`, caps saved notebooks/cells/SQL payloads, normalizes cell metadata, supports multiline `-- cell:` / `-- chart:` directives in the Fyne query editor, loads saved cells back into that editor, runs the newest saved notebook cell by cell, isolates failures, records successful SQL cells as SQL run/dependency lineage, and can export the latest run as a Markdown artifact with cell SQL, row previews, plans, chart SVG snippets, searchable metadata, and source freshness fingerprints. The Fyne notebook controls now keep the loaded notebook ID when saving again, expose a notebook label field, list parsed cells, and can move or delete selected cells while rewriting the directive text safely. The native SQLite workspace connector lives in `nexus-app/internal/services/dbconnector`; it opens selected `.sqlite`, `.sqlite3`, and `.db` files read-only, validates rooted non-symlink paths, inspects tables/views/columns/indexes/row counts/capped samples/relationship hints with a short timeout, and runs explicit guarded `SELECT`/`WITH` query previews with single-statement validation, mutation-token blocking, default 100-row/30-second bounds, SQL run metadata, dataset dependency lineage, and a native Cancel SQLite action that cancels the active context-aware query without stopping the application shell. SQLite connector queries can now be saved as bounded snippets under `.nexusdesk/datasets/queries.json`, listed back in the Data panel, exported as CSV artifacts, or exported as Markdown query reports with SQL, caps, rows, source metadata, SQL run rows, and dataset dependency lineage. SQL history can be reused back into the editor or rerun for the selected dataset/SQLite source from the Data tab. The dataset service builds deterministic SVG visuals from the latest bounded query result: categorical rows become bar charts by count or numeric sum, ordered date/time or numeric first columns with numeric values become line charts, and dashboard previews add KPI cards, a chart panel, and dataset notes around the same bounded result. The Fyne bottom Data tab owns grouped Source, Notebook, Visuals, and History action strips rather than one crowded row. It dispatches only explicit profile/query/SQL/connector/notebook/chart/dashboard/history/export intents plus read-only result rendering. The Add cell actions append valid SQL/chart notebook templates so users do not need to memorize directive syntax. Notebook runs now populate Data panel result tabs for Summary, Rows, Plan, and Charts; non-notebook outputs reset supplemental tabs to avoid stale evidence. `nexus-app/internal/services/artifacts` owns writing Markdown notebook run artifacts under `.nexusdesk/artifacts/notebooks`, SQLite query CSV/Markdown artifacts under `.nexusdesk/artifacts/sqlite-queries`, SVG chart artifacts under `.nexusdesk/artifacts/charts`, SVG dashboard artifacts under `.nexusdesk/artifacts/dashboards`, and source-path metadata/freshness fingerprints for those outputs. A fuller notebook editor surface, DuckDB multi-file SQL, richer SQLite lineage actions, external database profiles, Parquet value reads, and large-file profiling remain planned native ports.

Native Git status, selected-file diffs, parsed hunks, file-level stage/unstage actions, and index-only hunk stage/unstage actions live in `nexus-app/internal/services/git`. The service is manual-refresh/manual-select only, parses `git status --porcelain=v1 --branch`, splits staged and unstaged rows, validates selected repository-relative paths, loads capped staged and unstaged diffs, parses unified diff hunk metadata, runs selected-file `git add -- path` and `git restore --staged -- path`, applies selected hunks with `git apply --cached` or `git apply --cached --reverse`, and suppresses Windows command windows for spawned Git processes. The Fyne Git panel renders changed files grouped by directory, projects compact status badges into the Workbench tree from the last manual refresh, shows read-only unified, split, and diff-only selected-file diff views, exposes previous/next hunk navigation, confirms file-level and hunk-level stage/unstage actions, can send the selected diff to the native assistant service for an AI summary or commit-message draft, then refreshes repository status and the selected diff after mutations. Destructive hunk discard/revert, full-repository AI review, and broader approval-backed Git mutations remain later native ports.

Native task discovery and the first safe task-run boundary live in `nexus-app/internal/services/tasks`. Discovery is manual, bounded, skips noisy folders such as `.git`, `.nexusdesk`, build output, dist output, vendor, and `node_modules`, parses `package.json` scripts, detects Go module test commands, and lists Docker Compose config-check tasks. Running or retrying a task re-discovers the workspace task list, matches only a discovered task ID, validates the command shape, checks the working directory stays inside the workspace, runs with timeout and capped stdout/stderr, and hides Windows child-process windows. Native operations inspection lives in `nexus-app/internal/services/operations`; it manually scans for Dockerfiles, Compose, env, config, script, and log files, reads selected files through a bounded read-only path, redacts environment-like secret values, extracts lightweight Compose service details, and derives dependency/port/volume topology summaries without running Docker or shell commands. The Fyne bottom Operations tab owns scan, inspect, explicit Compose validation, and runbook-export intent plus read-only rendering. Compose validation is user-triggered, confirms before execution, resolves the selected Compose file to its discovered safe `docker compose config` task, then runs through the same job/task/report pipeline; folder open and scan still do not execute Docker or shell commands. Runbook exports re-inspect the selected operations file and write a cited Markdown artifact with the same topology summary without executing Docker, shell, or service commands. Native task runs now start through `nexus-app/internal/services/jobs`, which provides job IDs, status, log tail, completion state, cancellation contexts, and optional persistence. The native bottom Tasks tab can discover tasks, confirm a selected run, show status, and render the last captured stdout/stderr in a read-only output area; the Jobs tab lists recent task jobs, can request cancellation for running jobs, can retry completed jobs from persisted task-run records when the task is still discoverable, and can open generated task-report artifacts or fall back to persisted stdout/stderr. Completed task runs also write Markdown report artifacts and JSON sidecar metadata through `nexus-app/internal/services/artifacts`. The native Artifacts tab now uses that service for recursive generated-output listing, optional archived-output visibility, metadata search, archive/delete/restore actions, bounded previews, task-report lineage rendering, same-kind generated-output comparison diffs, comparison report exports, artifact-to-assistant context pinning, cited-source open/pin actions, source freshness warnings for missing, timestamp-modified, or same-timestamp fingerprint-changed cited files, document extraction report exports for Markdown/TXT/HTML/XML/DOCX/XLSX/PDF files through `nexus-app/internal/services/documents`, operations runbook exports for inspected Docker/Compose/env/config/log/script evidence, and first document-set Markdown report generation from the selected file, folder, or project root through the same bounded context-pack extraction path. Explicit artifact writes, refreshes, archives, restores, and deletes now upsert or remove matching rows in the native SQLite metadata store so generated outputs appear in metadata-backed history, while filesystem sidecars remain the artifact source of truth. The native Artifact surface is still smaller than the full Wails Artifact Studio because richer non-Markdown artifact writers and regeneration workflows remain future ports.

Native metadata persistence starts in `nexus-app/internal/services/metadata`. Opening a workspace ensures `.nexusdesk/metadata/schema.sql`, `.nexusdesk/metadata/sqlite-manifest.json`, and `.nexusdesk/metadata/nexusdesk.sqlite` exist through `modernc.org/sqlite`; the shell attaches that store to the Jobs and Approvals services so jobs reload per workspace and approval records prefer SQLite when available. The metadata service imports bounded Wails-era compatibility records: matching user-config chat history, workspace approval logs, generated artifact `.meta.json` sidecars, tool-run logs, and legacy Wails SQLite dataset SQL/dependency tables. Legacy dataset tables are detected by column shape, renamed to `legacy_wails_*` backup tables, recreated with the native schema, and converted into native `sql_runs` plus `dataset_dependencies` rows. Completed task runs are recorded in `task_runs` with command, cwd, status, exit code, bounded stdout/stderr, timestamps, duration, and `artifact_path` when a report file was created. Generated artifact rows are recorded in `artifacts` with kind, title, relative path, sidecar path, size, job/task IDs, source/source paths, archive state, and generated timestamps after explicit artifact writes, artifact refreshes, or compatibility imports. Approval decisions are recorded in `approval_records` with action, target, risk, decision, message, and timestamp while `.nexusdesk/approvals/log.json` remains the compatibility fallback. SQL runs are recorded in `sql_runs` with selected dataset path, SQL text, engine, status, row counts, message/error text, timing, and optional artifact path; dataset lineage records in `dataset_dependencies` link source datasets to SQL runs or later generated outputs. Native agent runs persist through `agent_runs` plus ordered `tool_runs`, capturing the prompt, job ID, status, final message, plan, source paths, iterations, stop reason, tool arguments, observations, errors, risk, mutation flag, and timestamps; imported legacy tool-run logs are grouped under one compatibility agent run so the Agent Audit surface can still inspect them.

Native workspace history starts in `nexus-app/internal/services/history`. It merges bounded chat search results, generated artifact rows from SQLite metadata, persisted SQL/dependency data records, persisted jobs, and persisted agent runs into one newest-first list with optional kind/query filters. For older workspaces with no artifact rows yet, history can fall back to artifact sidecar scans until the user refreshes Artifacts and the rows are upserted. The Fyne bottom History tab owns only list/detail rendering and jump actions: artifacts open in the Artifact preview/lineage flow, chats populate the Chat detail surface, data records show their SQL/dependency details, jobs refresh the Jobs tab, and agent runs open through the Agent Audit detail flow.
...
Native approvals now start in `nexus-app/internal/services/approvals`. The service writes append-only approval records to `.nexusdesk/approvals/log.json`, persists full-project access policy in `.nexusdesk/approvals/policy.json`, expires project trust by timestamp, records grant/revoke decisions, and uses an attached repository interface so the active SQLite metadata store can save and serve approval rows without coupling the approval service to SQLite. The Fyne bottom Approvals tab can refresh records, show full project access status, grant one hour of workspace-scoped project access, and revoke it. This policy is intentionally separate from shell execution and does not grant arbitrary command access.
...
- Data & Analytics has a first read-only SQLite workspace database schema-inspection surface.
...
- Expand native Data & Analytics into the full visual SQL notebook shell, then add SQLite connector cancellation, saved connector queries, CSV/Markdown exports, query-history filters, and selected-object schema explanation.

...
- `app/frontend/src/features/shell/CodeStudioPanel.tsx` owns reusable Code Studio session metrics, open tabs, workspace status, git branch/dirty summary, staged/unstaged changed-file groups, selected changed-file review state, per-file read-only staged/unstaged diffs, and placeholder queues for search, problems, tasks, and code review.

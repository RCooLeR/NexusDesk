...
- `app/frontend/src/features/shell/DataOperationsPanel.tsx` owns the Data & Analytics route surface for dataset profiling, query/chart/SQL workflows, read-only SQLite connector queries, manual SQLite connector schema inspection, Operations inspector, metadata browser/search, and workspace freshness controls.
...
- Expand the SQLite connector with saved connector queries, richer schema navigation, relationship hints, and clearer read-only status. The first manual schema inspection path is implemented.

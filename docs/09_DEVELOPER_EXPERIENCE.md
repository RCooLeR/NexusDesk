...
- `app/frontend/src/features/shell/CodeStudioPanel.tsx` owns reusable Workbench session metrics, open tabs, workspace status, git branch/dirty summary, staged/unstaged changed-file groups, selected changed-file state, the Workbench path/text/symbol/regex search utility panel with non-mutating replace previews, lightweight Problems results, read-only detected task listings, and placeholder queues for code review.

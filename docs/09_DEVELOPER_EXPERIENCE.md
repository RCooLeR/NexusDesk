...
- `app/frontend/src/features/shell/GitDiffPanel.tsx` owns the bottom-drawer Git tab for working-tree review, including selected changed-file state, per-file read-only staged/unstaged diffs, unified/split rendering, hunk navigation, and refresh controls.

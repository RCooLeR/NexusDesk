...
Native dataset profiling and row querying start in `nexus-app/internal/services/datasets`. The first slice profiles the selected workspace CSV, TSV, or JSON file through the existing preview-safe read boundary, reports sample row counts, field names, inferred lightweight types, empty/non-empty counts, sample values, JSON top-level shape, and truncation when the preview cap applies. The same service now runs bounded row queries over preview-safe CSV, TSV, and JSON rows with global text search, `column=value`, `column contains value`, numeric comparisons, `order by column asc|desc`, and `limit N`; query output is capped to 50 displayed rows and reports loaded, matched, and shown counts. The Fyne bottom Data tab owns only the "Profile selected" and "Run query" intents plus read-only profile/query rendering; SQL notebooks, connectors, charts, dependency records, exports, and large-file profiling remain planned native ports.

...
- Connector schema browsing now uses a shared frontend metadata browser for workspace SQLite and saved external database profile inspections, so external profile metadata can be navigated by table/view and inspected for columns, indexes, samples when present, and relationships.

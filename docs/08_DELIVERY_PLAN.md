...
5. Continue editor improvements toward search/problems/refactoring depth. Split editor groups, pinned tabs, breadcrumbs, outline/symbol navigation, the jumpable Document Map, safe draft formatting, and encoding-aware save are now implemented in the native editor-quality foundation; Monaco-specific go-to-definition is being replaced with deliberate Fyne-native equivalents.

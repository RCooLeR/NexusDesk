...
Current native Data & Analytics status: `nexus-app/internal/services/datasets` handles file dataset profiling/query/SQL/notebooks/charts, and `nexus-app/internal/services/dbconnector` now provides the first explicit read-only workspace SQLite schema inspector for `.sqlite`, `.sqlite3`, and `.db` files. Guarded SQLite connector SQL execution, cancellation, saved connector queries, CSV/Markdown exports, external database profiles, and connector lineage remain pending native migration items even though the preserved Wails reference contains versions of those workflows.

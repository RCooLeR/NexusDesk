...
| Artifact dependency rebuild/regeneration | `RebuildDatasetDependency` | Native can regenerate dataset query CSV, dataset SQL report, chart, dashboard, and SQL notebook run artifacts from dependency metadata | `ported` baseline | Continue broader regeneration coverage for summaries, SQLite exports, and future artifact kinds |
...
| Assistant retry, compare latest answer, save latest answer as artifact | `AgentPanel.tsx`, `CreateChatMarkdownArtifact` | Native Fyne surfaces retry, compare, and save-latest-answer; saved answers become `chat-answer` artifacts with prompt/model/context/source/citation metadata | `ported` baseline | Continue source-quality UX polish |
...
1. Finish the editor parity strategy: syntax highlighting and future LSP/deeper cross-file language choices.
2. Continue assistant quality polish: deeper retrieval evidence and source-quality UX.
3. Expand artifact regeneration beyond the dataset/query/chart/notebook rebuild baseline.

...
| Monaco syntax highlighting, language workers, minimap/editor outline UX | `MonacoFileEditor.tsx`, `MonacoCodePreview.tsx`, `editorOutline.ts`, frontend `dist/assets/*Mode*` | Native text editor supports editing, preview, dirty-close safety, quick-open, find/replace, and a Fyne outline tab ported from the Wails outline rules | `replace` | Finish syntax, breadcrumb, split/minimap, and language-action strategy; embed only after a focused spike proves packaging and accessibility |
...
1. Finish the editor parity strategy: syntax highlighting, breadcrumbs, encoding controls, split/editor layout decision, and minimap/language-action choices.

...
| Monaco syntax highlighting, language workers, minimap/editor outline UX | `MonacoFileEditor.tsx`, `MonacoCodePreview.tsx`, `editorOutline.ts`, frontend `dist/assets/*Mode*` | Native text editor supports editing, preview, dirty-close safety, quick-open, find/replace, Wails-derived breadcrumbs, explicit save encoding controls, deterministic Go/JSON formatting, Wails-style secondary split preview selection, and a Fyne outline tab ported from the Wails outline rules | `replace` | Finish syntax, minimap, and language-action strategy; embed only after a focused spike proves packaging and accessibility |

...
| Assistant retry, compare latest answer, save latest answer as artifact | `AgentPanel.tsx`, `CreateChatMarkdownArtifact` | Native Fyne surfaces retry, compare, and save-latest-answer; saved answers become `chat-answer` artifacts with prompt/model/context/source/citation/unverified-citation/snippet/evidence/coverage metadata | `ported` baseline | Continue deeper retrieval evidence polish |
...
| Presentation/report generation | Roadmap/docs references, artifact report foundation | First native presentation-outline artifacts can be generated from report-like artifacts in the Artifacts UI and regenerated from their source artifact metadata; packaged deck/PPTX-style exports remain future work | `ported` baseline | Continue richer generated document and packaged presentation exports after outline workflows prove useful |
| Packaging/build docs for Wails | `app-wails/README.md`, Wails build scripts | Active app is `nexus-app/`; Wails instructions are reference-only; native Windows/macOS/Linux CI smoke and ldflag-backed version metadata validation exist for the Fyne app | `drop` from primary path | Remove Wails build instructions from primary docs after freeze |

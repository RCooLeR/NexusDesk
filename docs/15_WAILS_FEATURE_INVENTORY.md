...
| Artifact dependency rebuild/regeneration | `RebuildDatasetDependency` | Native can regenerate dataset summary, dataset query CSV, dataset SQL report, chart, dashboard, SQL notebook run, SQLite query CSV/Markdown artifacts, workspace scan reports, and document-extraction artifacts from dependency/source metadata | `ported` baseline | Continue broader regeneration coverage for future artifact kinds |
...
3. Expand artifact regeneration beyond the current dataset summary/query/chart/notebook/SQLite/scan/document-extract rebuild baseline as future artifact kinds are added.

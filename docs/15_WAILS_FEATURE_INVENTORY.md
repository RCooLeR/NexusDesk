...
| Assistant retry, compare latest answer, save latest answer as artifact | `AgentPanel.tsx`, `CreateChatMarkdownArtifact` | Native Fyne surfaces retry, compare, and save-latest-answer; saved answers become `chat-answer` artifacts with prompt/model/context/source/citation/snippet/evidence metadata | `ported` baseline | Continue deeper retrieval evidence polish |
...
3. Expand artifact regeneration beyond the current dataset summary/query/chart/notebook/SQLite/scan/document-extract/runbook/comparison rebuild baseline as future artifact kinds are added.

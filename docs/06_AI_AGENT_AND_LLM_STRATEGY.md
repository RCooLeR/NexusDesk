...
- `app/internal/agent/` now contains a backend-first ReAct runtime that can plan, call tools, process observations, prune working memory, force a no-tool finalization pass when the iteration budget is exhausted, and return final answers through `RunAgent`.

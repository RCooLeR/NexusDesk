...
| Git | status, file diff, agent-readable diff context, changed-file previews, read-only history/blame context, hunk/file stage/unstage, branch creation, staged-change commits, rollback-backed conflict-marker resolution, assistant summaries | revert/cherry-pick and PR drafting/creation through connectors |

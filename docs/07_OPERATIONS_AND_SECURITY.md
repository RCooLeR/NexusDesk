...
Saved external database profiles are explicit-action only. PostgreSQL, MySQL/MariaDB, SQL Server, and DuckDB profile tests, schema inspections, and guarded queries run only after the user presses the matching Settings action or query command. Guarded profile queries carry request IDs so the app can cancel in-flight work, and every profile query path shares cap normalization, timeout normalization, and connector error redaction. Schema inspection may fetch capped sample rows through provider-quoted table names, but it does not run user-provided SQL and keeps samples at the shared metadata cap. DuckDB file profiles validate the target file in default builds and require a CGO-enabled `duckdb` tagged build for live execution.

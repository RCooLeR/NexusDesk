...
| Workspace | `read_context`, `read_file`, `search_workspace`, `read_problems`, `goto_definition`, `find_references`, `read_dependency_graph` | Bounded workspace reads, search, context packs, lightweight diagnostics, native outline/search-backed symbol navigation, and bounded code dependency graph scans. |

...
| Git | `read_git_status`, `read_git_diff`, `read_git_history`, `read_git_blame`, `stage_file`, `unstage_file`, `stage_hunk`, `unstage_hunk`, `commit_changes`, `create_branch`, `resolve_conflict`, `revert_changes` | Read-only repository context plus approval-gated index-only staging/unstaging, commits from already-staged changes, branch creation with optional checkout, rollback-backed conflict-marker resolution, and rollback-backed discard for unstaged tracked changes or explicit untracked-file deletion. Staged destructive resets remain planned. |
...
- `revert_staged_changes`: destructive staged reset/discard through explicit preview.

...
| Git | `read_git_status`, `read_git_diff`, `read_git_history`, `read_git_blame`, `stage_file`, `unstage_file`, `stage_hunk`, `unstage_hunk`, `commit_changes`, `create_branch` | Read-only repository context plus approval-gated index-only staging/unstaging, commits from already-staged changes, and branch creation with optional checkout. Destructive Git actions remain planned. |
...
- `resolve_conflict`: approved conflict-resolution patch.

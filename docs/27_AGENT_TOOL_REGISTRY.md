...
| Git | `read_git_status`, `read_git_diff`, `read_git_history`, `read_git_blame`, `stage_file`, `unstage_file`, `stage_hunk`, `unstage_hunk`, `commit_changes` | Read-only repository context plus approval-gated index-only staging/unstaging and commits from already-staged changes. Branch and destructive Git actions remain planned. |

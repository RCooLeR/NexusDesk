...
- editor quality: symbol search, diagnostics panel, and problems panel. Split editor groups, tab pinning, breadcrumbs, outline navigation, searchable go-to-symbol, the jumpable Document Map, safe draft formatting, and file encoding controls exist in the current foundation; deeper native go-to-definition remains planned.

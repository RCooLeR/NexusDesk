...
- editor quality: symbol search, diagnostics panel, and problems panel. Split editor groups, tab pinning, breadcrumbs, outline navigation, searchable go-to-symbol, local go-to-definition, the jumpable Document Map, safe draft formatting, and file encoding controls exist in the current foundation; future LSP-backed cross-file navigation remains planned.
- code search: path search, text search, regex, replace preview, symbol search, and saved searches
- tests/tasks: detect package scripts, Go tests, Python pytest, Cargo tests, npm scripts, Docker Compose tasks, run selected tasks with logs, and save run artifacts

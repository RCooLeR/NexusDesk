...
- The native editor is functional but not yet IDE-grade. Native outline, searchable go-to-symbol, jumpable Document Map, breadcrumbs, explicit save encoding controls, deterministic Go/JSON format actions, safe Markdown/config/SQL/Dockerfile/text whitespace formatting, Wails-style secondary split preview selection, and live find match counts have started, while syntax highlighting and deeper language-aware actions still need a deliberate editor strategy.
...
1. Native editor quality: syntax highlighting and deeper language-aware navigation.

...
- The native editor is functional but not yet IDE-grade. Native outline, breadcrumbs, explicit save encoding controls, and deterministic Go/JSON format actions have started, while syntax highlighting, richer find/replace, split groups, minimap, and language-aware actions still need a deliberate editor strategy.
...
1. Native editor quality: syntax highlighting, richer find/replace, split editor groups, minimap, broader formatting, and language-aware navigation.

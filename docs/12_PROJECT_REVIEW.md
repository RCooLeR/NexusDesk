...
- The native editor is functional but not yet IDE-grade. Native outline, breadcrumbs, explicit save encoding controls, deterministic Go/JSON format actions, and Wails-style secondary split preview selection have started, while syntax highlighting, richer find/replace, minimap, and language-aware actions still need a deliberate editor strategy.
...
1. Native editor quality: syntax highlighting, richer find/replace, minimap, broader formatting, and language-aware navigation.

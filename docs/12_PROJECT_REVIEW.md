...
- The native editor is functional but not yet IDE-grade. Native outline, breadcrumbs, explicit save encoding controls, deterministic Go/JSON format actions, Wails-style secondary split preview selection, and live find match counts have started, while syntax highlighting, minimap, and language-aware actions still need a deliberate editor strategy.
...
1. Native editor quality: syntax highlighting, minimap, broader formatting, and language-aware navigation.

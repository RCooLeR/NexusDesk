...
- [x] P1 Produce macOS app/package artifact. Evidence: `scripts/package-unix.sh` creates a macOS `NexusDesk.app` bundle archive with `Info.plist`, executable, README, and manifest/SBOM/provenance sidecars; hosted native CI run 26706200021 passed the macOS native Fyne smoke job on commit `ea93fc6` with the package script wired into `scripts/ci-unix.sh`.
- [x] P1 Produce Linux package artifact. Evidence: `scripts/package-unix.sh` creates a Linux tarball with `bin/nexusdesk`, desktop entry, icon payload, README, and manifest/SBOM/provenance sidecars; hosted native CI run 26706200021 passed the Linux native Fyne smoke job on commit `ea93fc6` with the package script wired into `scripts/ci-unix.sh`.
...
- [x] P1 Implement macOS signing/notarization if chosen. Evidence: `scripts/package-unix.sh` now produces a notarization-compatible macOS zip package and supports optional `--macos-sign-identity` / `NEXUSDESK_MACOS_CODESIGN_IDENTITY` plus `--macos-notary-profile` / `NEXUSDESK_MACOS_NOTARY_PROFILE`; the macOS path signs the app bundle, submits a notary upload through `xcrun notarytool`, staples the app, and then writes package manifest/SBOM/provenance sidecars. Hosted native CI run 26706324057 passed the unsigned macOS package path on commit `f2ab9c0`; credential-backed signing/notary execution remains release-operator evidence, not a CI secret requirement.
...
- [x] P1 Windows CI: tests. Evidence: hosted native CI run 26706979872 passed the Windows native Fyne smoke job on commit `4125d0e`, and `scripts/ci-windows.ps1` ran `go test ./...` including `internal/app` smoke-command coverage.
- [x] P1 Windows CI: build check. Evidence: hosted native CI run 26706979872 passed the Windows native Fyne smoke job on commit `4125d0e`; `scripts/ci-windows.ps1` built the native Windows executable, then ran `scripts/smoke-windows-installer.ps1` so the installed app executes `--version` and `--smoke-check` before uninstall cleanup.
- [x] P1 Windows CI: release manifest check. Evidence: hosted native CI run 26706979872 passed the Windows native Fyne smoke job on commit `4125d0e`; `scripts/ci-windows.ps1` generated manifest/SBOM/provenance release evidence for the native executable, and the nested Windows installer smoke generated and verified package-side manifest/SBOM/provenance evidence before running installed-app smoke.
...
- [x] P1 Linux CI: build/package smoke. Evidence: hosted native CI run 26707224450 passed the Linux native Fyne smoke job on commit `4d71dda`; `scripts/ci-unix.sh` builds the native executable, writes manifest/SBOM/provenance, creates the Linux package, extracts it through `scripts/smoke-unix-package.sh`, and runs the packaged `nexusdesk --version` plus `--smoke-check` core workspace/data/artifact/diagnostics flow.
...
- [x] P1 macOS CI: build/package smoke. Evidence: hosted native CI run 26707224450 passed the macOS native Fyne smoke job on commit `4d71dda`; `scripts/ci-unix.sh` builds the native executable, writes manifest/SBOM/provenance, creates the macOS app-bundle zip, extracts it through `scripts/smoke-unix-package.sh`, and runs the packaged app executable with `--version` plus `--smoke-check` core workspace/data/artifact/diagnostics flow.
...
- [x] P2 Add release notes link. Evidence: update guidance points users to `docs/releases/v1-release-notes.md` and `docs/releases/beta-release-notes.md`, and the command palette detail advertises release notes as part of the update-check path.
...
- [x] P0 Run full test suite in CI. Evidence: hosted native CI run 26707224450 passed the Windows, Linux, and macOS native Fyne smoke jobs on commit `4d71dda`; the jobs run gofmt, `go test ./...`, static analysis/build metadata checks where scripted, native executable builds, release-evidence generation, and packaged-app smoke checks for Windows installer, Linux package, and macOS app-bundle zip.
...
- [x] P0 Run accessibility review. Evidence: `docs/releases/accessibility-review.md` records the v1 accessibility review scope, local evidence, residual release blockers, and pass-with-blockers decision; focused accessibility/UI matrix passed across theme contrast diagnostics, rail shortcut/tooltips, command palette disabled/status text, visual smoke layouts, Settings, Welcome, warnings, progress/cancel states, and user guides (`go test ./internal/ui/theme ./internal/ui/shell ./internal/services/userguide -run "Palette|Contrast|Theme|Density|Focus|Tooltip|Shortcut|Keyboard|VisualSmoke|Disabled|Status|Warning|Error|Welcome|Settings|CommandPalette|ToolWindow|Smoke|Cancel|Progress" -count=1`).
...
- [x] P0 Publish release notes. Evidence: `docs/releases/v1-release-notes.md` publishes the v1 release-candidate notes with highlights, validation evidence, trust state, known limitations, required user actions, and remaining release blockers; `docs/README.md` links the notes as a release-specific source of truth; Help > Check for Updates points users to both v1 and beta notes (`go test ./internal/services/userguide ./internal/ui/shell -run "UpdateCheck|BetaFeedback|ReleaseHygiene|MainMenuHelpGroupExposesUpdateCheck|CommandPaletteIncludesSafeAgentGuide" -count=1`).

...
- Notebook runs keep cell failures isolated, render per-cell rows, plan lines, chart summaries, and messages, persist successful SQL cells into SQL run/dependency metadata, and can be exported as Markdown artifacts with per-cell SQL, row previews, plan lines, chart SVG snippets, source metadata, and freshness fingerprints. The fuller Wails-era visual notebook shell with cell tabs, result tabs, and history reuse/rerun controls is still a post-port item.

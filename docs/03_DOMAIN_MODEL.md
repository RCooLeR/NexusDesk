...
- The native Fyne Data panel now uses a multiline notebook editor syntax: `-- cell: Label` starts a SQL cell and `-- chart: Label` starts a chart cell. Saving persists multiple cells, loading restores those directives, and `Run notebook` executes the newest saved notebook cell by cell through the bounded dataset SQL runner.
- Notebook runs keep cell failures isolated, render per-cell rows, plan lines, chart summaries, and messages, persist successful SQL cells into SQL run/dependency metadata, and can be exported as Markdown artifacts with per-cell SQL, row previews, plan lines, chart SVG snippets, source metadata, and freshness fingerprints. The Data panel can reuse or rerun the latest SQL history entry for the selected dataset or SQLite source, separates notebook output into Summary, Rows, Plan, and Charts tabs, and includes first visual controls for inserting SQL/chart cell templates. The fuller Wails-era visual notebook shell with cell selection, reorder/delete controls, and richer explain output is still a post-port item.

...
- The latest consolidated review raises production blockers that should be handled before broad new feature work: preview truncation data-loss safety, XLSX/DOCX/PPTX decompression caps, macOS Keychain argv hardening, Windows DPAPI buffer pinning, DNS-rebinding SSRF hardening, workspace search performance, connector TLS defaults, metadata WAL/busy timeout, streaming/activity throttling, editor save threading, and UI shell controller extraction.
...
- Agent tools for catalog inspection, context, search, problems, approval-gated native file formatting with diff preview/encoding preservation/rollback, medium-risk native file lint diagnostics, native outline/search-backed go-to-definition/reference lookup, bounded symbol index reads, approved reviewed project memory writes with source provenance, plus bounded Go/JavaScript/TypeScript/Python/stylesheet dependency graph scans, Git reads, approval-gated Git index stage/unstage for files and hunks, approval-gated commits from already-staged changes, approval-gated branch creation with optional checkout, rollback-backed conflict-marker resolution, rollback-backed discard for unstaged tracked changes or explicit untracked deletion, approval-gated staged-only discard with explicit `scope=staged`, staged diff preview, rollback-backed file restore/delete, and mixed-edit rejection, local dataset profile/query/SELECT-only SQL/chart artifact generation, workspace SQLite schema/query, document extraction, read-only operations scan/inspection/runbook generation, tasks, durable job list/log/cancel, first-party approved terminal commands, security redaction/approval primitives, artifacts/lineage/regeneration, safe file mutations, rollback, web fetch, detection-only external coding-agent CLI readiness for Codex, Claude Code, and OpenCode, and non-executing external-agent run planning.
...
- Direct dispatcher wrappers for remaining already-built service capabilities are currently complete; native symbol-index, dependency-graph, and approved project-memory tools are now available; continue hardening new service capabilities as they land.
...
- Richer terminal/session work beyond one-shot approved commands only after shell policy presets, durable process supervision, audit UX, cancellation, and output capture are mature.
...
- Artifact provenance health gate that checks generated outputs for readable metadata sidecars and lineage signals in Diagnostics.
...
- Crash/hang checks for folder open, malformed files, corrupt metadata, missing providers, and canceled long work. The readiness failure-scenario matrix now pins automated/manual evidence references and is visible in Diagnostics; keep adding scenario-specific tests as new workflows land.
...
- Large `data_panel.go`: split work has landed and Data panel state now lives in a focused controller; continue extracting remaining Settings/small shell ownership clusters.
...
- [ ] Define shell state ownership boundaries: workspace and settings remain to formalize; editor, assistant, data, artifacts, jobs, diagnostics, search, Git, and rollbacks now have focused controller ownership.
...
- [ ] Keep external coding-agent CLI execution optional and secondary; implement only after NexusDesk's own file, Git, terminal, browser, connector, and artifact tools are stable and after sandboxing, redaction, prompt/permission UX, rollback/artifact capture, and clean cancellation behavior are finalized.

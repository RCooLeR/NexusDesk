...
Workspace tree actions call the same service boundary: the Fyne navigator action strip and selected-item action menu collect user intent and confirmation, then dispatch create, copy, rename/move, and delete through `nexus-app/internal/services/workspace` so rollback and rooted validation stay centralized. Clipboard/path and assistant-context affordances stay UI-only helpers; they do not bypass the workspace service for mutations.

...
Persisted conversation history follows the metadata boundary too. `nexus-app/internal/services/metadata` owns chat message writes, recent conversation loading, and bounded chat search; the Fyne shell owns only the assistant sidebar preview, bottom Chat tab presentation, and user intent to seed the assistant composer from a prior record. Search and detail rendering never read the SQLite database directly from widgets, and follow-up actions reuse the same assistant context-pin state rather than bypassing the assistant service.

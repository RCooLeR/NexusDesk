...
Workspace tree actions call the same service boundary: the Fyne navigator quick-action strip and tree-row secondary-click context menus collect user intent and confirmation, then dispatch file create, folder create, copy, paste-as-copy, paste-as-move, rename/move, and delete through `nexus-app/internal/services/workspace` so rollback and rooted validation stay centralized. The tree is lazy by default, has explicit expand/collapse controls, and can show normally ignored folders only through a user toggle; `.nexusdesk` metadata and symlinks remain hidden even in that diagnostic view. Clipboard/path and assistant-context affordances stay UI-only helpers; pasted file mutations still enter the workspace service at apply time.
...
- `nexus-app/internal/services/tools/` owns deterministic tool descriptors, argument parsing, dispatcher handlers, and the native tool catalog for implemented and planned first-party LLM tools. Current executable handlers cover catalog inspection, context, preview/search/problems, native symbol/dependency context, approved project-memory writes, Git reads, approval-gated index-only staging/unstaging, staged-only commits, safe branch creation, local dataset profile/query/SELECT-only SQL/chart generation, workspace SQLite schema/query, document extraction, read-only operations scan/inspection/runbook generation, tasks, durable job list/log/cancel, approved terminal commands, rollbacks, safe mutations, artifact lineage/regeneration, web fetch, and external-agent readiness/planning; remaining browser, MCP, automation, and richer Git/terminal wrappers are tracked in `docs/27_AGENT_TOOL_REGISTRY.md`.
...
- First-party terminal execution is part of the native agent surface only as an approved, one-shot, non-shell argv tool; richer terminal sessions and external coding-agent launch remain future work.

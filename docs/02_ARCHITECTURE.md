...
- `nexus-app/internal/services/tools/` owns deterministic tool descriptors, argument parsing, dispatcher handlers, and the native tool catalog for implemented and planned first-party LLM tools. Current executable handlers cover catalog inspection, context, preview/search/problems, Git reads, local dataset profile/query/SELECT-only SQL, workspace SQLite schema/query, document extraction, read-only operations scan/inspection, tasks, approved terminal commands, rollbacks, safe mutations, artifact lineage/regeneration, web fetch, and external-agent readiness/planning; remaining operations/job/data artifact, browser, MCP, automation, and richer Git/terminal wrappers are tracked in `docs/27_AGENT_TOOL_REGISTRY.md`.

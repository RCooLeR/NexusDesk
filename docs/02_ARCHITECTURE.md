...
- `nexus-app/internal/services/tools/` owns deterministic tool descriptors, argument parsing, dispatcher handlers, and the native tool catalog for implemented and planned first-party LLM tools. Current executable handlers cover catalog inspection, context, preview/search/problems, Git reads, approval-gated index-only staging/unstaging, staged-only commits, local dataset profile/query/SELECT-only SQL/chart generation, workspace SQLite schema/query, document extraction, read-only operations scan/inspection/runbook generation, tasks, durable job list/log/cancel, approved terminal commands, rollbacks, safe mutations, artifact lineage/regeneration, web fetch, and external-agent readiness/planning; remaining browser, MCP, automation, and richer Git/terminal wrappers are tracked in `docs/27_AGENT_TOOL_REGISTRY.md`.

...
- Fix the remaining P0 Claude/Codex review items before broad new feature work: continue UI shell controller extraction beyond the completed Search, Jobs, Rollbacks, and Diagnostics controller slices, then add any follow-up search cancellation/debouncing needed in the shell controller layer.
...
- [ ] C-1.5 UI shell `View` is a god object: Search, Jobs, Rollbacks, and Diagnostics panel state/actions are now isolated in controllers; continue extracting controllers/state for Data, Assistant, Git, Editor, Artifacts, and Settings.

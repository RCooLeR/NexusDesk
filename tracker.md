...
- [x] Agent tools for read context, workspace search, problems, Git status/diff/history/blame, local dataset profile/query/SELECT-only SQL, document extraction, read-only operations scan/inspection, tasks, first-party approved terminal commands, artifact lineage/regeneration, safe writes, patches, copy/move/delete, and rollback.
...
- [ ] Add direct dispatcher wrappers for remaining already-built service capabilities: SQLite inspect/query, operations runbook export, artifact-backed dataset chart generation, and job list/log/cancel.

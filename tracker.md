...
- The latest Claude findings are now consolidated into `docs/29_FULL_PROJECT_REVIEW_AND_ROADMAP.md`; the top production risks are UI shell controller extraction, connector TLS defaults, metadata WAL/busy timeout, streaming/activity throttling, editor-save threading, signed packaging, and platform protected-secret smoke.
...
- Fix the remaining P0 Claude/Codex review items before broad new feature work: continue UI shell controller extraction beyond the completed Search, Jobs, Rollbacks, Diagnostics, Git, Artifacts, Assistant, Editor, Data, Settings, and Operations controller slices, then add any follow-up search cancellation/debouncing needed in the shell controller layer.
...
- [x] Add medium-risk native `update_project_memory` agent tool with approval, redaction, and source provenance.
...
- [x] Assistant source digest: inspect latest-answer evidence quality, verified refs, unverified refs, cited sources, and uncited sources from a native Assistant dialog.
...
- [x] Native IDE-style status bar with workspace/index health, provider/model readiness, last manual Git branch, job counts, warning count, selected path, editor encoding, line ending, and app version.
...
- [x] Refactor native Settings into searchable grouped preferences with inline readiness validation for provider, runtime, secrets, token budgets, and task model routes.
...
- [x] Add native `goto_definition` and `find_references` agent tools using safe text reads, Wails-derived outline rules, and bounded workspace search fallback.
...
- [x] First durable job monitor with progress log tail, cancellation, retry from persisted task runs, task-report output opening, and visible persistence warnings when job repository saves fail.
...
- [x] Surface production failure gates in first-run readiness and Diagnostics so release-candidate smoke can verify the evidence matrix from the app.
...
- [ ] C-1.5 UI shell `View` is a god object: Search, Jobs, Rollbacks, Diagnostics, Git, Artifacts, Assistant, Editor, Data, Settings, and Operations panel state/actions are now isolated in controllers; continue extracting controller/state ownership for remaining small shell clusters.

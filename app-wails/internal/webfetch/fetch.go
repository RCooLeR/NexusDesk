...
func Fetch(ctx context.Context, request Request) (Result, error) {
	...
	httpRequest.Header.Set("User-Agent", "NexusAugenticStudio/agent-web-fetch")
...
func extractTitle(text string, contentType string) string {
	mediaType, _, _ := mime.ParseMediaType(contentType)
	if strings.EqualFold(mediaType, "text/html") || strings.EqualFold(mediaType, "application/xhtml+xml") {
		// The title may already be stripped, so fall through to the first line.
	}

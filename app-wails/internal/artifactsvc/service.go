...
func (s *Service) CreateMarkdownReport(relPath string) (artifact.MarkdownReport, error) {
	...
	if relPath != "" {
		preview, err := workspace.Preview(root, relPath, workspace.PreviewOptions{MaxBytes: previewMaxBytes})
		if err != nil {
			return artifact.MarkdownReport{}, err
		}
		source = preview
...
func (s *Service) CreateScanReport() (artifact.MarkdownReport, error) {
...
func (s *Service) CreateGeneratedMarkdown(request artifact.MarkdownArtifactRequest) (artifact.MarkdownReport, error) {
...
func (s *Service) List() ([]artifact.WorkspaceArtifact, error) {
...
func (s *Service) Metadata(relPath string) (artifact.ArtifactMetadata, error) {
...
func (s *Service) Archive(relPath string) (artifact.MarkdownReport, error) {
...
func (s *Service) Delete(relPath string) (artifact.MarkdownReport, error) {
...
func (s *Service) Compare(leftRelPath string, rightRelPath string) (artifact.ArtifactComparison, error) {
...
func (s *Service) requireRoot(action string) (string, error) {
	root := s.workspaceRoot()
	if root == "" {
...
func (s *Service) persist(root string, relPath string) {
	if s.persistMetadata != nil {
		s.persistMetadata(root, relPath)
	}
}

func (s *Service) record(action string, target string, risk string, message string) {

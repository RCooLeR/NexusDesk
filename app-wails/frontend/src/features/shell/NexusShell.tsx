...
const accessPolicyStorageKey = 'nexus:access-policy';
...
export function NexusShell({
    ...
    const [accessPolicy, setAccessPolicy] = useState<AccessPolicy>(readAccessPolicy);
    const [accessPolicyStatus, setAccessPolicyStatus] = useState(accessPolicySummary(accessPolicy));
    ...
    }, [llmSettings, probeResult?.runtime]);

    useEffect(() => {
        if (accessPolicy.projectFiles !== 'full' || !accessPolicy.workspaceRoot || accessPolicy.workspaceRoot === workspace?.root) {
            return;
        }
        const next = {...accessPolicy, projectFiles: 'approval' as const, workspaceRoot: ''};
        setAccessPolicy(next);
        writeAccessPolicy(next);
        setAccessPolicyStatus('Project full access was reset because the active workspace changed.');
    }, [accessPolicy, workspace?.root]);
    ...
    function startNewFileDraft() {
        ...
        if (relPath.toLowerCase().startsWith('.nexusdesk/')) {
            setWorkspaceStatus('Direct writes to Nexus metadata are not allowed.');
            return;
        }
    ...
    async function moveActiveFile() {
        ...
        if (targetRelPath.toLowerCase().startsWith('.nexusdesk/')) {
            setWorkspaceStatus('Direct moves into Nexus metadata are not allowed.');
            return;
        }
    ...
    async function handleTreeContextAction(action: TreeContextAction, node: FileNode) {
        await selectWorkspaceNode(node);
        if (action === 'copy-path') {
            try {
                await navigator.clipboard?.writeText(node.relPath);
                setWorkspaceStatus(`${node.relPath} copied to clipboard.`);
                pushToolEvent('Path copied', node.relPath);
            } catch {
                setWorkspaceStatus(`Copy path: ${node.relPath}`);
            }
            return;
        }
        if (action === 'new-file') {
        ...
        if (action === 'new-folder') {
            setWorkspaceStatus('Folder creation preview is queued for the file operation batch.');
            pushToolEvent('Tree action queued', `New folder under ${node.relPath}`);
            return;
        }
        if (action === 'rename' || action === 'move') {
            setWorkspaceStatus(`${action === 'rename' ? 'Rename' : 'Move'} selected ${node.relPath}. Use the command palette action to preview the safe mutation.`);
            setIsCommandPaletteOpen(true);
            setCommandPaletteQuery('rename');
            return;
        }
        if (action === 'delete') {
            setWorkspaceStatus(`Delete selected ${node.relPath}. Use the command palette action to preview the high-risk mutation.`);
            setIsCommandPaletteOpen(true);
            setCommandPaletteQuery('delete');
            return;
        }
        setWorkspaceStatus(`Reveal ${node.relPath} selected. Native reveal will land behind a backend shell-safe action.`);
        pushToolEvent('Tree reveal queued', node.relPath);
    }
    ...
    function updateSettingsDraft(field: keyof LLMSettings, value: string) {
        setSettingsDraft((current) => {
            if (field === 'model') {
                return settingsForSelectedModel(current, value, probeResult?.runtime);
            }
            ...
            if (field === 'maxContextTokens') {
                const maxContextTokens = Number.parseInt(value, 10) || current.maxContextTokens;
                return {
                    ...next,
    ...
    function changeChatModel(model: string) {
        const next = settingsForSelectedModel(settingsDraft, model, probeResult?.runtime);
        setSettingsDraft(next);
        setSettingsStatus(`Using ${model} with ${next.maxContextTokens.toLocaleString()} context and ${next.responseReserveTokens.toLocaleString()} response reserve.`);
        pushToolEvent('Model context maxed', `${model}: ctx ${next.maxContextTokens.toLocaleString()}, reserve ${next.responseReserveTokens.toLocaleString()}`);
    ...
    async function testLLMConnection() {
        ...
        try {
            ...
            const tunedSettings = settingsWithRuntimeContext(settingsDraft, result.runtime);
            if (tunedSettings.maxContextTokens !== settingsDraft.maxContextTokens || tunedSettings.responseReserveTokens !== settingsDraft.responseReserveTokens) {
                setSettingsDraft(tunedSettings);
                pushToolEvent('Runtime context detected', `${tunedSettings.model}: ctx ${tunedSettings.maxContextTokens.toLocaleString()}, reserve ${tunedSettings.responseReserveTokens.toLocaleString()}`);
                void saveLLMSettings(tunedSettings);
            }
            if (result.ok) {
    ...
    async function updateAccessPolicy(field: keyof AccessPolicy, value: AccessPolicy[keyof AccessPolicy]) {
        if (accessPolicy[field] === value) {
            return;
        }
        if (field === 'projectFiles' && value === 'full') {
            if (!workspace) {
                setAccessPolicyStatus('Open a workspace before granting full project access.');
                return;
            }
            const approved = await requestApproval({
                action: 'Grant full project access',
                confirmLabel: 'Grant access',
                message: `Allow trusted agent runs to create or update files inside ${workspace.root} through Nexus safe write tools without asking again each run.`,
                risk: 'high',
                target: workspace.root,
            });
            if (!approved) {
                setAccessPolicyStatus('Full project access was not granted.');
                return;
            }
        }
        if (field === 'dataSources' && value === 'full') {
            const approved = await requestApproval({
                action: 'Grant full data-source read access',
                confirmLabel: 'Grant access',
                message: 'Allow trusted data workflows to inspect and query configured sources through read-only guarded connectors, result caps, and timeouts without extra policy prompts.',
                risk: 'medium',
                target: workspace?.root ?? 'Configured data sources',
            });
            if (!approved) {
                setAccessPolicyStatus('Full data-source access was not granted.');
                return;
            }
        }
        if (field === 'shellCommands' && value === 'approval') {
            const approved = await requestApproval({
                action: 'Enable shell approval policy',
                confirmLabel: 'Enable policy',
                message: 'Mark shell execution as approval-only for future operations flows. Chat agent shell execution remains disabled until dedicated Ops controls exist.',
                risk: 'high',
                target: workspace?.root ?? 'Operations policy',
            });
            if (!approved) {
                setAccessPolicyStatus('Shell approval policy was not enabled.');
                return;
            }
        }
        const next = {
            ...accessPolicy,
            [field]: value,
            workspaceRoot: field === 'projectFiles' && value === 'full' ? workspace?.root ?? '' : field === 'projectFiles' ? '' : accessPolicy.workspaceRoot,
        };
        setAccessPolicy(next);
        writeAccessPolicy(next);
        setAccessPolicyStatus(accessPolicySummary(next));
        pushToolEvent('Access policy updated', accessPolicySummary(next));
    }

    function resetAccessPolicy() {
        const next = defaultAccessPolicy();
        setAccessPolicy(next);
        writeAccessPolicy(next);
        setAccessPolicyStatus('Guarded workspace access is active.');
        pushToolEvent('Access policy reset', 'Project writes and data-source access require explicit approvals again.');
    }
    ...
    async function runAgentPrompt(approveFileWrites = false) {
        if (!workspace) {
        ...
        const projectFullAccess = accessPolicy.projectFiles === 'full' && accessPolicy.workspaceRoot === workspace.root;
        const promptNeedsWrite = promptRequestsWorkspaceWrite(prompt);
        let approveAgentWrites = approveFileWrites || projectFullAccess;
        if (!approveAgentWrites && promptNeedsWrite) {
            const approved = await requestApproval({
                action: 'Run agent with file write access',
                confirmLabel: 'Allow writes',
                message: `This prompt appears to ask Nexus to create or update a workspace file. Allow this agent run to write inside ${workspace.root} through safe write tools. Shell commands remain disabled.`,
                risk: 'high',
                target: workspace.root,
            });
            if (!approved) {
                setChatStatus('Agent write access was not approved, so no file was created.');
                return;
            }
            approveAgentWrites = true;
        }
        if (approveFileWrites && !projectFullAccess && !promptNeedsWrite) {
            const approved = await requestApproval({
                action: 'Run agent with file write access',
                confirmLabel: 'Allow writes',
                message: 'Allow this agent run to create or update files inside the active workspace through Nexus safe write tools. Shell commands remain disabled.',
                risk: 'high',
                target: workspace.root,
            });
            if (!approved) {
                setChatStatus('Agent write access was not approved.');
                return;
            }
        }
        ...
        try {
            const result = await RunAgent({
                requestId,
                prompt,
                approveHighImpact: approveAgentWrites,
                allowShellCommands: false,
            });
            replaceChatMessage(assistantMessage.createdAt, formatAgentRunResult(result), 'agent', agentRunSourcePaths(result));
            await refreshAgentToolRuns();
            const stepLabel = `${result.iterations} model step${result.iterations === 1 ? '' : 's'}`;
            if (result.stopReason) {
                setChatStatus(`Agent paused after ${stepLabel}; review the answer.`);
                pushToolEvent('Agent stopped', `${result.stopReason}; ${result.toolCalls.length} tool calls`);
            } else {
                setChatStatus(`Agent completed ${stepLabel}.`);
    ...
    async function summarizeSelectedContext() {
        ...
        try {
            await sendPromptText(prompt, {
                ...
                saveArtifactSource: 'Nexus summary',
    ...
    async function sendPromptText(rawPrompt: string, options: SendPromptOptions) {
        ...
        const contextDetail = contextPaths.length > 0
            ? `${contextPaths.length} context path${contextPaths.length === 1 ? '' : 's'}`
            : 'no explicit context';
        const budgetDetail = `${settingsDraft.model}: ctx ${settingsDraft.maxContextTokens.toLocaleString()}, reserve ${settingsDraft.responseReserveTokens.toLocaleString()}`;

        setIsSendingPrompt(true);
        setChatStatus(contextRelPath ? `Streaming with ${contextRelPath} as context...` : 'Streaming without selected file context...');
        setChatMessages((current) => [...current, userMessage, assistantMessage]);
        pushToolEvent('Model request queued', `${budgetDetail}; ${contextDetail}`);
        if (options.clearComposer) {
            setChatPrompt('');
        }

        const unsubscribe = listenForChatStream(requestId, assistantMessage.createdAt, contextRelPath);
        pushToolEvent(unsubscribe ? 'Stream listener attached' : 'Streaming unavailable', unsubscribe ? requestId : 'Falling back to one-shot chat request.');
        try {
            pushToolEvent('Context budget reserved', `${Math.max(0, settingsDraft.maxContextTokens - settingsDraft.responseReserveTokens).toLocaleString()} tokens available before backend byte caps.`);
            ...
            if (options.saveArtifactTitle && workspace) {
                const report = await writeAssistantArtifact({
                    ...
                    model: result.model,
                    prompt,
                    source: options.saveArtifactSource ?? 'Nexus chat',
            ...
            pushToolEvent('Chat failed', message || 'The provider did not return a usable chat response.');
        } finally {
            unsubscribe?.();
            setIsSendingPrompt(false);
        }
    ...
    function renderStudioPanel(
        ...
        options: {ariaLabel?: string; className?: string; showTabs?: boolean} = {}
    ) {
        return (
            <BottomStudioPanel
                activeTab={activeTab}
                ...
                accessPolicyStatus={accessPolicyStatus}
                ...
                onAccessPolicyChange={(field, value) => void updateAccessPolicy(field, value)}
                ...
                onResetAccessPolicy={resetAccessPolicy}
    ...
    return (
        <div
            className={`app-shell studio-route-${activeStudioRoute}`}
            data-active-studio-route={activeStudioRoute}
            style={{
        ...
        >
            ...
            <WorkspaceNavigator
                ...
                onSelectSearchResult={(result) => void selectSearchResult(result)}
                onTreeContextAction={(action, node) => void handleTreeContextAction(action, node)}
            ...
            {mainStudioTab === null ? (
                <WorkbenchPanel
                    ...
                    workspace={workspace}
                />
            ) : renderStudioPanel(mainStudioTab, {
                ariaLabel: `${studioRouteLabels[activeStudioRoute]} main surface`,
                className: 'main-studio-panel',
                showTabs: false,
            })}
            ...
            <AgentPanel
                ...
                onRunAgent={(approveFileWrites) => void runAgentPrompt(approveFileWrites)}
            ...
            <div
                ...
                role="separator"
            />
            {renderStudioPanel(activeBottomTab)}
        </div>
    );
}
...
function normalizeLLMSettings(settings: Partial<LLMSettings>, fallback: LLMSettings): LLMSettings {
    ...
    const defaultContextTokens = modelContextWindow(model);
    const maxContextTokens = numericLLMSetting(settings.maxContextTokens, numericLLMSetting(fallback.maxContextTokens, defaultContextTokens));
...
function formatAgentRunResult(result: AgentRunResultView) {
    ...
    if (result.stopReason === 'context_limit' || result.stopReason === 'context_limit_finalized') {
        return 'The selected model context filled before the agent produced a final answer. See Activity for the latest model and tool events.';
    }
    if (result.stopReason === 'safety_guard' || result.stopReason === 'safety_guard_finalized') {
        return 'The local agent safety guard paused a repeated tool loop before a final answer. See Activity for the latest model and tool events.';
    }
    if (result.stopReason === 'iteration_limit' || result.stopReason === 'iteration_limit_finalized') {
        return 'The agent paused before producing a final answer. See Activity for the latest model and tool events.';
    }
...
function formatAgentRunEvent(event: AgentRunEvent) {
    ...
    case 'finalizing':
        return `${prefix} Wrapping up with completed tool results.`;
    case 'stopped_finalized':
        return `${prefix} Wrapped up: ${truncateInline(event.message || 'final answer ready', 220)}`;
    case 'stopped':
        return `${prefix} Paused: ${truncateInline(event.error || event.message || 'no final answer', 220)}`;
    case 'final':
...
function agentEventPrefix(event: AgentRunEvent) {
    const step = event.iteration > 0 ? `step ${event.iteration}` : 'run';
    return `- ${step}`;
}
...
function agentRunStatus(event: AgentRunEvent) {
    switch (event.type) {
    case 'model_request':
        return `Agent step ${event.iteration}: waiting for model.`;
    case 'model_response':
        return `Agent step ${event.iteration}: model responded.`;
    case 'tool_start':
    ...
    case 'finalizing':
        return 'Agent is wrapping up with completed results.';
    case 'stopped':
    case 'stopped_finalized':
        return 'Agent paused.';
    case 'final':
...
function buildAgentToolPlan(
    ...
    if (preview?.kind === 'file' && !preview.table && !isArtifactPath(preview.relPath)) {
        ...
        add('workspace.delete', preview.relPath, 'requires confirmation');
    }
...
function defaultAccessPolicy(): AccessPolicy {
    return {
        projectFiles: 'approval',
        dataSources: 'approval',
        shellCommands: 'disabled',
        workspaceRoot: '',
    };
}

function readAccessPolicy(): AccessPolicy {
    try {
        const parsed = JSON.parse(window.localStorage.getItem(accessPolicyStorageKey) || '{}') as Partial<AccessPolicy>;
        return normalizeAccessPolicy(parsed);
    } catch {
        return defaultAccessPolicy();
    }
}

function writeAccessPolicy(policy: AccessPolicy) {
    try {
        window.localStorage.setItem(accessPolicyStorageKey, JSON.stringify(policy));
    } catch {
        // Ignore storage failures; the current session still has the selected policy.
    }
}

function normalizeAccessPolicy(policy: Partial<AccessPolicy>): AccessPolicy {
    return {
        projectFiles: policy.projectFiles === 'full' ? 'full' : 'approval',
        dataSources: policy.dataSources === 'full' ? 'full' : 'approval',
        shellCommands: policy.shellCommands === 'approval' ? 'approval' : 'disabled',
        workspaceRoot: String(policy.workspaceRoot ?? ''),
    };
}

function accessPolicySummary(policy: AccessPolicy) {
    const project = policy.projectFiles === 'full' ? `full project file access${policy.workspaceRoot ? ` for ${policy.workspaceRoot}` : ''}` : 'approval for project writes';
    const data = policy.dataSources === 'full' ? 'full read access to data sources' : 'approval for data-source work';
    const shell = policy.shellCommands === 'approval' ? 'shell approval policy marked' : 'shell disabled';
    return `${project}; ${data}; ${shell}.`;
}

function promptRequestsWorkspaceWrite(prompt: string) {
    const normalized = prompt.toLowerCase();
    if (/(do not write|don't write|without writing|no file|not to a file)/.test(normalized)) {
        return false;
    }
    const mutationVerbs = '(create|write|save|append|update|modify|edit|change|fix|implement|refactor|rename|move|remove|delete|patch|apply|generate|document|record|list|cleanup|clean\\s+up)';
    const workspaceTargets = '(file|document|report|artifact|markdown|\\.md|tracker|findings?|code|source|bug|bugs|issue|issues|app|project|workspace|ui|backend|frontend|component|module|docs?|tests?)';
    return [
        new RegExp(`\\b${mutationVerbs}\\b[\\s\\S]{0,120}\\b${workspaceTargets}\\b`),
        new RegExp(`\\b${workspaceTargets}\\b[\\s\\S]{0,120}\\b${mutationVerbs}\\b`),
        /\b(to|into|in)\s+(a\s+|the\s+)?(new\s+)?(file|document|report|artifact|markdown)\b/,
        /\b[a-z0-9_.-]+\.md\b/,

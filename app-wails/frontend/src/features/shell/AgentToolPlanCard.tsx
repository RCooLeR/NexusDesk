                    ...
                    {planItems.map((item) => (
                        <div className="tool-plan-item" key={`${item.toolName}-${item.target}`}>
                            ...
                            <div className="tool-plan-actions">
                                ...
                                <Button disabled={isRunning || requiresExternalToolInput(item.toolName)} onClick={() => onExecute(item)} variant="subtle">Execute</Button>
                            </div>
                        </div>
                    ))}
                </div>
...
function requiresExternalToolInput(toolName: string) {
    return ['workspace.write', 'workspace.writeBinary', 'workspace.patch', 'workspace.copy', 'workspace.move'].includes(toolName);
}

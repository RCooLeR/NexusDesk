...
func TestProfileStoreReturnsDefaultsWhenMissing(t *testing.T) {
	store := NewProfileStore(filepath.Join(t.TempDir(), "assistant-profile.json"))
...
func TestProfileStoreSavesMemoryAndActiveProfile(t *testing.T) {
	store := NewProfileStore(filepath.Join(t.TempDir(), "assistant-profile.json"))
...
func TestProfileStoreNormalizesInvalidProfiles(t *testing.T) {
	store := NewProfileStore(filepath.Join(t.TempDir(), "assistant-profile.json"))
...
func TestApplyProfileToPromptIncludesProfileAndMemory(t *testing.T) {
	prompt := ApplyProfileToPrompt("Summarize the report.", Profile{
		Memory:          "Prefer concise answers.",
		ActiveProfileID: "reviewer",
		PromptProfiles:  DefaultProfile().PromptProfiles,
	})
	for _, expected := range []string{
		"user-provided guidance, not source evidence",
		"Active prompt profile: Reviewer",

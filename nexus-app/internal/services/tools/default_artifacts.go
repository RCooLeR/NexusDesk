...
func (h defaultHandlers) rebuildArtifact(ctx context.Context, root string, store *artifactsSvc.Store, original artifactsSvc.Artifact) (artifactsSvc.Artifact, error) {
	...
	case "presentation-package":
		source, ok := artifactRegenerationSource(original)
		if !ok {
			return artifactsSvc.Artifact{}, fmt.Errorf("presentation package artifact %s has no source outline metadata", original.RelPath)
		}
		return rebuildPresentationPackageArtifact(ctx, store, source)
	case "presentation-deck":
		source, ok := artifactRegenerationSource(original)
		if !ok {
			return artifactsSvc.Artifact{}, fmt.Errorf("presentation deck artifact %s has no source outline metadata", original.RelPath)
		}
		return rebuildPresentationDeckArtifact(ctx, store, source)
	default:
		return artifactsSvc.Artifact{}, fmt.Errorf("artifact kind %q cannot be regenerated by the agent tool", original.Kind)
	}
}

func rebuildPresentationOutlineArtifact(ctx context.Context, store *artifactsSvc.Store, source string) (artifactsSvc.Artifact, error) {
	if err := ctx.Err(); err != nil {
		return artifactsSvc.Artifact{}, err
	}
...
func rebuildPresentationDeckArtifact(ctx context.Context, store *artifactsSvc.Store, source string) (artifactsSvc.Artifact, error) {
	if err := ctx.Err(); err != nil {
		return artifactsSvc.Artifact{}, err
	}
	metadata, err := store.ReadArtifactMetadata(source)
	...
	return store.WritePresentationDeckReport(artifactsSvc.BuildPresentationDeckReport(
		"",
		source,
		metadata.Title,
		metadata.Kind,
...
func rebuildPresentationPackageArtifact(ctx context.Context, store *artifactsSvc.Store, source string) (artifactsSvc.Artifact, error) {
	if err := ctx.Err(); err != nil {
		return artifactsSvc.Artifact{}, err
	}
	metadata, err := store.ReadArtifactMetadata(source)
	...
	return store.WritePresentationPackageReport(artifactsSvc.BuildPresentationPackageReport(
		"",
		source,
		metadata.Title,
		metadata.Kind,
